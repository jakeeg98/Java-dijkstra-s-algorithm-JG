...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        ...
        if(start == null || end == null){
            throw new NullPointerException("One or more of the keys are null");
        }
        //Make sure the inputs exist in our graph
        if(!vertices.containsKey(start) || !vertices.containsKey(end)){
            throw new NoSuchElementException("The keys do not exist!");
        }
        //Create a priority queue and initliaze our first vertex with the start value
        PriorityQueue<Path> frontier = new PriorityQueue<>();
        Vertex first = vertices.get(start);

        //Create a path and add it to the frontier
        Path original = new Path(first);
        frontier.add(original);
        //Create a hashtable to check what verticies have already been visited
        Hashtable<T, Vertex> visited = new Hashtable<>();
        while(!frontier.isEmpty()){
            //Get the best path from the frontier
            Path newPath = frontier.poll();
            //Make sure we are not visiting something again
            if(visited.containsKey(newPath.end.data)){
                continue;
            }
            //If the end data of this path equals what we are looking for, it should be the shortest oath
            if(newPath.end.data.equals(end)){
                return newPath;
            }else{
                //If not, add this to our map of visited verticies
                visited.put(newPath.end.data, newPath.end);
                //Loop through all of the possible edges
                for(int i = 0; i < newPath.end.edgesLeaving.size(); i++){
                    //If it has already been visited skip to next iteration
                    if(visited.containsKey(newPath.end.edgesLeaving.get(i).target.data)){
                        continue;
                    }
                    //If not, create a new path by adding on to our existing path
                    Path curr = new Path(newPath, newPath.end.edgesLeaving.get(i));
                    //Then add this to our frontier so the loop can continue
                    frontier.add(curr);
                }
            }
        }
        //In the case that no path could be found, an exception should be thrown
        throw new NoSuchElementException("Could not find a path!");
    }
